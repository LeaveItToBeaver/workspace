...
class UserRepository {
    ...
    async findByZipCode(zipCode) {
        try {
            ...
            if (typeof usersRef.orderByChild === 'function' && typeof usersRef.equalTo === 'function') {
                const snapshot = await usersRef.orderByChild('zipCode').equalTo(zipCode).once('value');
                const users = snapshot.val() || {};
