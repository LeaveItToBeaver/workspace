        ...
        if (editingId === user.id && editingField === "name") {
          return (
            <EditableCell
              value={String(value)}
              onSave={(newValue) =>
                handleFieldUpdate(user.id, "name", newValue)
              }
      ...
      cell: (value, user) => {
        if (editingId === user.id && editingField === "zipCode") {
          return (
            <EditableCell
              value={String(value)}
              onSave={(newValue) =>
                handleFieldUpdate(user.id, "zipCode", newValue)
              }
              onCancel={handleFieldCancel}
              onFieldSwitch={handleFieldCancel}
              placeholder="10001"
        ...
        return (
          <button
            ...
            title={
              editingId === user.id
                ? "Click to edit zip code (will refresh location data)"
                : ""
            }
          >
            {String(value)}
      ...
      {editingId && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <p className="text-sm text-blue-800">
            <strong>Edit Mode:</strong> Click on Name or Zip Code fields to edit
            them directly. Press Enter to save, Escape to cancel current field.
            {editingField === null &&
              " Click Cancel to exit edit mode completely."}
          </p>
        </div>
      )}
